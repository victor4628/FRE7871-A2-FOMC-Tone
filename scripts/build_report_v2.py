...
def build():
    ...
    story+=[tbl(rm,[2.2*inch,1.4*inch,1.4*inch],align_right=(1,2)),Spacer(1,7),
            ...
            P(f"<b>Recommendation:</b> {forecast['recommendation']['position']} {forecast['recommendation']['rationale']} <b>What would make this wrong:</b> {forecast['recommendation']['falsifier']}",'Call'),
            P('Remaining neutral is itself the pre-meeting position. It reflects weak directional evidence rather than a claim that no market move will occur. The forecast is an academic exercise, not investment advice.'),PageBreak(),
