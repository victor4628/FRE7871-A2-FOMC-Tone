...
def build():
    ...
    story+=[Spacer(1,6),tbl(validation,[2.45*inch,.8*inch,.8*inch,.8*inch],small=True,align_right=(1,2,3)),Spacer(1,7),P(f"Statement tone: <b>{forecast['more_hawkish_probability']}%</b> probability that the September statement is more hawkish than July 29. This is obtained by weighting historical tone-change frequencies after cuts, holds, and hikes by the internally predicted decision probabilities.",'Call'),P('Expected meeting-day market reaction','H3x')]
    ...
    story+=[tbl(rm,[2.2*inch,1.4*inch,1.4*inch],align_right=(1,2)),Spacer(1,7),
            ...
            P('The position is deliberately small because the market regressions are noisy. Its value is the consistency of the curve direction across all three statement-score specifications, not a claim of high confidence. The forecast is an academic pre-meeting position, not investment advice.'),PageBreak(),
            ...
            P('Data: Federal Reserve document and event calendars; FRED T10Y2Y, DGS1, and DGS3MO; Yahoo Finance DX-Y.NYB, IWF, and IWN. Models: a disclosed RoBERTa-large temporal reproduction, ProsusAI/finbert, and an internally validated pre-meeting multinomial logit. No market-implied decision probability is used. Full links and provenance appear in README.md and METHODOLOGY.md.','Smallx')]
